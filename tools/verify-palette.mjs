...
for (const accessor of previous.json.accessors) {
  ...
  const a = previous.json.bufferViews[index],
    b = current.json.bufferViews[index];
  assert.deepEqual(
    { ...b, byteOffset: 0 },
    { ...a, byteOffset: 0 },
  ...
  assert.ok(
    previous.bin
      .subarray(a.byteOffset, a.byteOffset + a.byteLength)
...
console.log(
  `Palette-only export verified: ${current.json.accessors.length} accessors, nodes, meshes, skins and animations match the preserved source exactly.`,
);
