...
export class AileronControls {
  ...
  private readonly surfaces: { bone: T.Object3D; motion: T.QuaternionLinearInterpolant }[];
  constructor(model: T.Object3D, animations: T.AnimationClip[]) {
    const clip = animations.find((candidate) => candidate.name === "AileronBank");
    ...
    this.surfaces = ["L", "R"].map((side) => {
      ...
      if (!bone || !track) throw new Error(`Blender control surface missing: ${name}`);
      return { bone, motion: new T.QuaternionLinearInterpolant(track.times, track.values, 4) };
    });
    this.reset();
  }
  reset() { this.amount = 0; this.apply(0); }
  private apply(amount: number) {
    const time = (amount + 1) * .5 * this.duration;
  ...
  update(dt: number, steering: number, transform: number) {
    ...
    const deployed = 1 - T.MathUtils.smoothstep(transform, 0, .25);
